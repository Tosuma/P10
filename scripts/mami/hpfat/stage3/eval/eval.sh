...
for ndre in $(seq -f "%.1f" 0.0 0.1 1.0); do
  for ndvi in $(seq -f "%.1f" 0.0 0.1 1.0); do
    MODEL_NAME="checkpoints/hpfat/stage3/hpfat-andhra-stage3-re_${ndre}-vi_${ndvi}_stage3_best.pth"
    RES_DIR="results/hpfat/andhra-stage3---Weedy-Rice/re_${ndre}_vi_${ndvi}"
