...
run_eval \
  --model "./checkpoints/basemodel-kaz-new/stage1_best_final.pth" \
  --pred "results/basemodel-new-stage1---Sri-Lanka/data/" \
  --truth "$sri_path" \
  --type "Sri-Lanka" \
  --out "results/basemodel-new-stage1---Sri-Lanka/results.json" \
  --save-images

# Base model stage 1 (trained on Kazakhstan) test on Weedy Rice
run_eval \
  --model "./checkpoints/basemodel-kaz-new/stage1_best_final.pth" \
  --pred "results/basemodel-new-stage1---Weedy-Rice/data/" \
  --truth "$weedy_path" \
  --type "Weedy-Rice" \
  --out "results/basemodel-new-stage1---Weedy-Rice/results.json" \
  --save-images