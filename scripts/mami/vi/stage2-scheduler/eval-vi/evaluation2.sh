...
for ndre in $(seq -f "%.1f" 0.6 0.1 1.0); do
  for ndvi in $(seq -f "%.1f" 0.0 0.1 1.0); do
    dir_name="results/vi/stage2-scheduler---sri-lanka/re_${ndre}_vi_${ndvi}"
    model_name="./checkpoints/vi/finals/stage2-scheduler/vi-weedy-scheduler-re_${ndre}-vi_${ndvi}_stage2_best.pth"
