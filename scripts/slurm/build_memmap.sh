#!/bin/bash
#SBATCH --job-name=build_memmap
#SBATCH --output=logs/build_memmap_%j.out
#SBATCH --error=logs/build_memmap_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
singularity exec --nv \
    ...
    /bin/bash -lc "PYTHONPATH=$SLURM_SUBMIT_DIR:$VENV_SITE \
        python -u $SLURM_SUBMIT_DIR/utils/build_memmap.py \
            --patch-dir $DATA_ROOT \
            --workers 15"
